...
int write_binary_su3_vector_data(su3_vector * const s,
		LemonWriter * lemonwriter, DML_Checksum *checksum,
		int const prec, const int t0) {
...
#else /* HAVE_LIBLEMON */
int write_binary_su3_vector_data(su3_vector * const s, 
		LimeWriter * limewriter, DML_Checksum * checksum,
		const int prec, const int t0, int const nsets) {
	int x, X, y, Y, z, Z, t, tag=0, id=0, i=0, status=0, sets = 0, skip = g_nproc_x*LX*g_nproc_y*LY*g_nproc_z*LZ;
	int latticeSize[] = {1, g_nproc_x*LX, g_nproc_y*LY, g_nproc_z*LZ};
	su3_vector tmp[1];
	float tmp2[24];
	int coords[4];
                  ...
                  for(y = 0; y < LY*g_nproc_y; y++) {
                          ...
                          for(x = 0; x < LX*g_nproc_x; x++) {
                                  ...
                                  if(g_cart_id == 0) {
                                          ...
                                          if(g_cart_id == id) {
                                                  if(prec == 32) {
                                                          be_to_cpu_assign_double2single((float*)tmp2, s + i + sets*skip, sizeof(su3_vector)/8);
                                                          DML_checksum_accum(checksum,rank,(char *) tmp2,sizeof(su3_vector)/2);
                                                          status = limeWriteRecordData((void*)tmp2, &bytes, limewriter);
                                                  }
                                                  else {
                                                          be_to_cpu_assign(tmp, s + i + sets*skip, sizeof(su3_vector)/8);
                                                          DML_checksum_accum(checksum,rank,(char *) tmp,sizeof(su3_vector));
                                                  ...
                                                  if(prec == 32) {
                                                          MPI_Recv((void*)tmp2, sizeof(su3_vector)/8, MPI_FLOAT, id, tag, g_cart_grid, &mstatus);
                                                          DML_checksum_accum(checksum,rank,(char *) tmp2, sizeof(su3_vector)/2);
                                                          status = limeWriteRecordData((void*)tmp2, &bytes, limewriter);
                                                  }
                                                  else {
                                                          MPI_Recv((void*)tmp, sizeof(su3_vector)/8, MPI_DOUBLE, id, tag, g_cart_grid, &mstatus);
                                                          DML_checksum_accum(checksum,rank,(char *) tmp, sizeof(su3_vector));
                                                          status = limeWriteRecordData((void*)tmp, &bytes, limewriter);
                                                  }
                                          }
#endif
                                          if(status < 0 ) {
                                                  fprintf(stderr, "LIME write error occurred with status = %d, while in write_binary_su3_vector_data_l (su3_vector_write_binary.c)!\n", status);
#ifdef MPI
                                          ...
                                          if(g_cart_id == id){
                                                  if(prec == 32) {
                                                          be_to_cpu_assign_double2single((float*)tmp2, s + i + sets*skip, sizeof(su3_vector)/8);
                                                          MPI_Send((void*) tmp2, sizeof(su3_vector)/8, MPI_FLOAT, 0, tag, g_cart_grid);
                                                  }
                                                  else {
                                                          be_to_cpu_assign(tmp, s + i + sets*skip, sizeof(su3_vector)/8);
                                                          MPI_Send((void*) tmp, sizeof(su3_vector)/8, MPI_DOUBLE, 0, tag, g_cart_grid);
                                                  }
                                          }
                                  }
#endif
	...
	if (g_debug_level > 0) {
		...
		if (g_cart_id == 0) {
			engineering(measure, latticeSize[1] * latticeSize[2] * latticeSize[3] * bytes, "b");
			fprintf(stdout, "# Time spent writing %s ", measure);
			engineering(measure, tock - tick, "s");
			fprintf(stdout, "was %s.\n", measure);
			engineering(measure, latticeSize[1] * latticeSize[2] * latticeSize[3] * bytes / (tock - tick), "b/s");
			fprintf(stdout, "# Writing speed: %s", measure);
			engineering(measure, latticeSize[1] * latticeSize[2] * latticeSize[3] * bytes / (g_nproc * (tock - tick)), "b/s");
			fprintf(stdout, " (%s per MPI process).\n", measure);
			fflush(stdout);
		}
	}
