...
int read_su3_vector(su3_vector * const s, char * filename, const int position_, const int t0) {
	int status = 0, read_data = 0, getpos = 0, bytes = 0, prec = 0, prop_type, position = position_, rstat=0;
	int read_checksum = 0, status_checksum = 0, position_checksum = position, getpos_checksum = 0;
	int read_eigenvalue = 0, status_eigenvalue = 0, position_eigenvalue = 0, getpos_eigenvalue = 0, timeslice = 0;
	...
	while ((status = ReaderNextRecord(reader)) != LIME_EOF) {
		...
		if(strcmp("eigenvalue-info", header_type) == 0 && read_eigenvalue == 0) {
			if(position_eigenvalue == getpos_eigenvalue) {
				...
				if(status_eigenvalue != LIME_SUCCESS) {
					...
					status_eigenvalue = parse_eigenvalue_xml(buffer, &eigenvalue, &timeslice);
					if(status_eigenvalue == 0) {
						fprintf(stderr, "extracting eigenvalue failed with return value %d\n", status_eigenvalue);
					} else {
						if(t0 != timeslice) {
							fprintf(stderr, "eigenvector of the wrong timeslice!\nAborting...");
							return(-9);
						}
					}
				}
		...
		if (strcmp("scidac-binary-data", header_type) == 0 && read_data == 0) {
			if (getpos == position) {
				...
				if ((int)bytes == LX * g_nproc_x * LY * g_nproc_y * LZ * g_nproc_z * sizeof(su3_vector)) {
					prec = 64;
				}
				else {
					if ((int)bytes == LX * g_nproc_x * LY * g_nproc_y * LZ * g_nproc_z * sizeof(su3_vector) / 2) {
						prec = 32;
					}
					else {
				...
				if( (rstat = read_binary_su3_vector_data(s, reader, &checksum, t0)) != 0) {
					fprintf(stderr, "read_binary_su3_vector_data failed with return value %d\n", rstat);
					return(-7);
				}
		...
		if(read_data && read_checksum && read_eigenvalue) break;
	}
	...
	if(read_checksum) {
		if(checksum_read.suma != checksum.suma && checksum_read.sumb != checksum.sumb) {
