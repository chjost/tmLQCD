...
void create_source_tf_df_lf(const int nr_conf, const int nr_dilution,
  ...
  tmp = (spinor*) calloc(VOLUME + 1, sizeof(spinor));
  if (tmp == NULL ) {
    free(rnd_vector);
    free(eigenvector);
  ...
  for (tslice = 0; tslice < T; tslice++) {
    for (vec = 0; vec < no_eigenvalues; vec++) {
      ...
      zero_spinor_field(dirac0, VOLUME);
      zero_spinor_field(dirac1, VOLUME);
      zero_spinor_field(dirac2, VOLUME);
      zero_spinor_field(dirac3, VOLUME);
      ...
      for (point = 0; point < SPACEVOLUME; point++) {
        _vector_add_mul( dirac0[SPACEVOLUME*tslice + point].s0,
            rnd_vector[index+0], eigenvector[point]);
        _vector_add_mul( dirac1[SPACEVOLUME*tslice + point].s1,
            rnd_vector[index+1], eigenvector[point]);
        _vector_add_mul( dirac2[SPACEVOLUME*tslice + point].s2,
            rnd_vector[index+2], eigenvector[point]);
        _vector_add_mul( dirac3[SPACEVOLUME*tslice + point].s3,
...
void create_source_tf_df_lb(const int nr_conf, const int nr_dilution,
  ...
  int status = 0, block = LX * LY * LZ;
  int rnd_vec_size = T * 4 * no_eigenvalues;
  int count = 0, v = 0, vs = dilution_list[nr_dilution].size[2];
  su3_vector * eigenvector = NULL;
  spinor *tmp = NULL;
  spinor *even = NULL, *odd = NULL;
  spinor *dirac0 = NULL;
...
void create_source_tn_df_lf(const int nr_conf, const int nr_dilution,
    char* inverterpath) {
  char filename[200];
  char call[150];
  int tslice = 0, vec = 0, point = 0, j = 0;
...
void create_source_tn_df_ln(const int nr_conf, const int nr_dilution,
    char* inverterpath) {
  char filename[200];
  char call[150];
  int tslice = 0, vec = 0, point = 0, j = 0;
  ...
  // write spinor field with entries at dirac 0
  convert_lexic_to_eo(even, odd, dirac0);
  sprintf(filename, "%s.%04d.%02d.%02d", "source0", nr_conf, 0, 0);
#if DEBUG
  printf("writing file %s\n", filename);
#endif
  construct_writer(&writer, filename, 0);
  ...
  // write spinor field with entries at dirac 1
  convert_lexic_to_eo(even, odd, dirac1);
  sprintf(filename, "%s.%04d.%02d.%02d", "source1", nr_conf, 0, 0);
#if DEBUG
  printf("writing file %s\n", filename);
#endif
  construct_writer(&writer, filename, 0);
  ...
  // write spinor field with entries at dirac 2
  convert_lexic_to_eo(even, odd, dirac2);
  sprintf(filename, "%s.%04d.%02d.%02d", "source2", nr_conf, 0, 0);
#if DEBUG
  printf("writing file %s\n", filename);
#endif
  construct_writer(&writer, filename, 0);
  ...
  // write spinor field with entries at dirac 3
  convert_lexic_to_eo(even, odd, dirac3);
  sprintf(filename, "%s.%04d.%02d.%02d", "source3", nr_conf, 0, 0);
#if DEBUG
  printf("writing file %s\n", filename);
#endif
  construct_writer(&writer, filename, 0);
...
void create_source_tn_df_li(const int nr_conf, const int nr_dilution,
    char* inverterpath) {
  char filename[200];
  char call[150];
  int tslice = 0, vec = 0, point = 0, j = 0, v = 0;
  int status = 0, block = LX * LY * LZ;
  int rnd_vec_size = T * 4 * no_eigenvalues;
  int count = 0;
  su3_vector * eigenvector = NULL;
...
void create_source_tn_df_lb(const int nr_conf, const int nr_dilution,
    char* inverterpath) {
  char filename[200];
  char call[150];
  int tslice = 0, vec = 0, point = 0, j = 0;
  int v = 0, vs = dilution_list[nr_dilution].size[2];
  int status = 0, block = LX * LY * LZ;
  int rnd_vec_size = T * 4 * no_eigenvalues;
  int count = 0;
  su3_vector * eigenvector = NULL;
...
void create_source_tb_df_lf(const int nr_conf, const int nr_dilution,
    char* inverterpath) {
  char filename[200];
  char call[150];
  int tslice = 0, vec = 0, point = 0, j = 0;
  int status = 0, block = LX * LY * LZ;
  int rnd_vec_size = T * 4 * no_eigenvalues;
  int count, t = 0, ts = dilution_list[nr_dilution].size[0];
  su3_vector * eigenvector = NULL;
  spinor *tmp = NULL;
  spinor *even = NULL, *odd = NULL;
  spinor *dirac0 = NULL;
...
void create_source_tb_df_ln(const int nr_conf, const int nr_dilution,
    char* inverterpath) {
  char filename[200];
  char call[150];
  int tslice = 0, vec = 0, point = 0, j = 0;
  int status = 0, block = LX * LY * LZ;
  int rnd_vec_size = T * 4 * no_eigenvalues;
  int count = 0, t = 0, ts = dilution_list[nr_dilution].size[0];
  su3_vector * eigenvector = NULL;
  spinor *tmp = NULL;
  spinor *even = NULL, *odd = NULL;
  spinor *dirac0 = NULL;
  ...
  for (tslice = 0; tslice < T; tslice++) {
    for (vec = 0; vec < no_eigenvalues; vec++) {
      ...
      for (point = 0; point < SPACEVOLUME; point++) {
        _vector_add_mul( dirac0[SPACEVOLUME*tslice + point].s0,
            rnd_vector[index+0], eigenvector[point]);
        _vector_add_mul( dirac0[SPACEVOLUME*tslice + point].s1,
            rnd_vector[index+1], eigenvector[point]);
        _vector_add_mul( dirac0[SPACEVOLUME*tslice + point].s2,
            rnd_vector[index+2], eigenvector[point]);
        _vector_add_mul( dirac0[SPACEVOLUME*tslice + point].s3,
            rnd_vector[index+3], eigenvector[point]);
      }
  ...
  for (tslice = 0; tslice < T; tslice++) {
    ...
    for (vec = 0; vec < no_eigenvalues; vec++) {
      ...
      for (point = 0; point < block; point++) {
        _vector_add_mul( dirac0[block*tslice + point].s0, rnd_vector[index+0],
            eigenvector[point]);
        _vector_add_mul( dirac0[block*tslice + point].s1, rnd_vector[index+1],
            eigenvector[point]);
        _vector_add_mul( dirac0[block*tslice + point].s2, rnd_vector[index+2],
            eigenvector[point]);
        _vector_add_mul( dirac0[block*tslice + point].s3, rnd_vector[index+3],
            eigenvector[point]);
      }
    }
...
void create_source_tn_dn_lf(const int nr_conf, const int nr_dilution,
  ...
  create_input_files(1, 0, nr_conf, nr_dilution, 0);
  sprintf(call, "%s -f dirac0.%d-cg.input 1> /dev/null", inverterpath, 0);
#if DEBUG
  printf("\n\ntrying: %s for conf %d, t %d (full)\n", call, nr_conf, tslice);
#endif
...
void create_source_tn_dn_li(const int nr_conf, const int nr_dilution,
  ...
  char filename[200];
  char call[150];
  int tslice = 0, vec = 0, point = 0, v = 0;
  int status = 0, block = LX * LY * LZ;
  int rnd_vec_size = T * 4 * no_eigenvalues;
  int count = 0;
  su3_vector * eigenvector = NULL;
